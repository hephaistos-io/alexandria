...
# Per-key TTL: 7 days. Conflict events are unlikely to reappear after a week,
# but downstream consumers should also guard against duplicates.
_DEDUP_TTL_SECONDS = 7 * 24 * 60 * 60

# Upper bound for the in-memory fallback. Conflict sources typically return
# hundreds to low thousands of events per fetch.
...
class RedisSeenUrls:
    ...
    Each event is stored as a key like ``seen:conflict:<source>:<source_id>``
    with a 7-day TTL.  Individual keys age out independently — unlike a single
