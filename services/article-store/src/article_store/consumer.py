...
DEFAULT_CONSUME_QUEUE = "articles.training"
...
class MessageConsumer:
    ...
    in-flight at a time — storage is I/O-bound on the DB write, no benefit
    to buffering multiple messages.
    """
    ...
    def _handle_delivery(
        ...
        Always ACKs — even on failure. Storage failures (bad JSON, constraint
        violations) won't succeed on retry, so leaving them unacked would
        poison the queue.
        """
        try:
