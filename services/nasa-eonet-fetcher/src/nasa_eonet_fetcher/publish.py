"""Publish natural disaster events to a RabbitMQ queue."""
...
DEFAULT_QUEUE = "natural_disasters.raw"
...
class RabbitMqPublisher:
    """Publishes NaturalDisaster objects to a RabbitMQ queue as JSON messages.

    Designed to be used as the on_event callback for SmartFetchLoop:

        publisher = RabbitMqPublisher("amqp://guest:guest@localhost:5672")
        loop = SmartFetchLoop(fetcher, on_event=publisher.publish)
    ...
    def publish(self, event: NaturalDisaster) -> None:
        """Serialize a NaturalDisaster to JSON and publish it to the queue.

        Matches the Callable[[NaturalDisaster], None] signature that SmartFetchLoop expects.
