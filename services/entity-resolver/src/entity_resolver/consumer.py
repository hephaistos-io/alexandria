...
connection when heartbeats are missed during long-running entity resolution).
"""
...
DEFAULT_CONSUME_QUEUE = "articles.tagged"
...
class MessageConsumer:
    ...
    in-flight at a time — entity resolution is I/O-bound (Redis + HTTP),
    but there is no benefit to buffering multiple messages at once.

    If the connection drops (missed heartbeats during slow Wikidata lookups),
    the consumer reconnects and resumes consuming automatically.
    """
    ...
    def _connect(self) -> None:
        """Open a fresh connection and channel, declare the queue and register consumer."""
    ...
    def _reconnect(self) -> None:
        """Reconnect with retry loop. Handles repeated failures gracefully."""
